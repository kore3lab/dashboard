...
export default {
	...
	watch: {
		...
		selected() {
			...
			localStorage.setItem('columns_hpa',this.selected)
		}
	},
	layout: "default",
	created() {
		...
		this.$nuxt.$on("navbar-context-selected", (ctx) => {
			if(localStorage.getItem('columns_hpa')) {
				this.selected = (localStorage.getItem('columns_hpa')).split(',')
			} else {
		...
		onRowSelected(items) {
			if(items) {
				if(items.length) {
					...
					this.viewModel = this.getViewLink('autoscaling', 'horizontalpodautoscalers', items[0].namespace, items[0].name)
