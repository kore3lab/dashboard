			...
			<div class="container-fluid">
				...
				<div class="row mb-2">
					...
					<div class="col-sm-1 text-right">
						<b-button variant="primary" size="sm" @click="$router.push(`/create?context=${currentContext()}&group=Configuration&crd=ConfigMap`)">Create</b-button>
					</div>
				</div>
			</div>
		</div>
...
export default {
	...
	watch: {
		selected() {
			...
			localStorage.setItem('columns_configmap',this.selected)
		}
	},
	created() {
		this.columnOpt = Object.assign([],this.fields)
		this.$nuxt.$on("navbar-context-selected", (ctx) => {
			if(localStorage.getItem('columns_configmap')) {
				this.selected = (localStorage.getItem('columns_configmap')).split(',')
			} else {
	...
	methods: {
		...
		onRowSelected(items) {
			if(items) {
				if(items.length) {
					...
					this.viewModel = this.getViewLink('', 'configmaps', items[0].namespace, items[0].name)
		...
		query_All() {
			this.isBusy = true;
			this.$axios.get(this.getApiUrl("","configmaps",this.selectedNamespace))
					.then((resp) => {
						this.items = [];
