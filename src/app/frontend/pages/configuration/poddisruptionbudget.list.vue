...
export default {
	...
	watch: {
		...
		selected() {
			...
			localStorage.setItem('columns_poddisruptionbudget',this.selected)
		}
	},
	layout: "default",
	created() {
		...
		this.$nuxt.$on("navbar-context-selected", (ctx) => {
			if(localStorage.getItem('columns_poddisruptionbudget')) {
				this.selected = (localStorage.getItem('columns_poddisruptionbudget')).split(',')
			} else {
