...
export default {
	...
	watch: {
		selected() {
			...
			localStorage.setItem('columns_limitrange',this.selected)
		}
	},
	created() {
		this.columnOpt = Object.assign([],this.fields)
		this.$nuxt.$on("navbar-context-selected", (ctx) => {
			if(localStorage.getItem('columns_limitrange')) {
				this.selected = (localStorage.getItem('columns_limitrange')).split(',')
			} else {
