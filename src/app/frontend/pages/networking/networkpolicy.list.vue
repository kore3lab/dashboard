	...
	<div class="content-wrapper">
		<div class="content-header">
			<div class="container-fluid">
				...
				<div class="row mb-2">
					<!-- title & search -->
					<div class="col-sm"><h1 class="m-0 text-dark"><span class="badge badge-info mr-2">N</span>Network Policies</h1></div>
					...
					<div class="col-sm-1 text-right">
						<b-button variant="primary" size="sm" @click="$router.push(`/create?context=${currentContext()}&group=Networking&crd=NetworkPolicy`)">Create</b-button>
					</div>
				</div>
			</div>
		</div>
...
export default {
	...
	watch: {
		selected() {
			...
			localStorage.setItem('columns_networkpolicy',this.selected)
		}
	},
	created() {
		this.columnOpt = Object.assign([],this.fields)
		this.$nuxt.$on("navbar-context-selected", (ctx) => {
			if(localStorage.getItem('columns_networkpolicy')) {
				this.selected = (localStorage.getItem('columns_networkpolicy')).split(',')
			} else {
	...
	methods: {
		...
		onRowSelected(items) {
			if(items) {
				if(items.length) {
					...
					this.viewModel = this.getViewLink('networking.k8s.io', 'networkpolicies', items[0].namespace, items[0].name)
		...
		query_All() {
			this.isBusy = true;
			this.$axios.get(this.getApiUrl("networking.k8s.io","networkpolicies",this.selectedNamespace))
					.then((resp) => {
						this.items = [];
