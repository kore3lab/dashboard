...
export default {
	...
	watch: {
		selected() {
			...
			localStorage.setItem('columns_endpoint',this.selected)
		}
	},
	created() {
		this.columnOpt = Object.assign([],this.fields)
		this.$nuxt.$on("navbar-context-selected", (ctx) => {
			if(localStorage.getItem('columns_endpoint')) {
				this.selected = (localStorage.getItem('columns_endpoint')).split(',')
			} else {
	...
	methods: {
		...
		onRowSelected(items) {
			if(items) {
				if(items.length) {
					...
					this.viewModel = this.getViewLink('', 'endpoints', items[0].namespace, items[0].name)
		...
		query_All() {
			this.isBusy = true;
			this.$axios.get(this.getApiUrl("","endpoints",this.selectedNamespace))
					.then((resp) => {
						this.items = [];
