				...
				<div class="d-flex flex-row-reverse">
					<div class="p-2">
						<b-form inline>
							...
							<span class="text-sm align-middle"><i class="fas fa-question-circle" id="limitMark"></i> Total : {{ totalItems }}</span>
...
export default {
	...
	watch: {
		...
		selected() {
			...
			localStorage.setItem('columns_event',this.selected)
		}
	},
	layout: "default",
	created() {
		...
		this.$nuxt.$on("navbar-context-selected", (ctx) => {
			if(localStorage.getItem('columns_event')) {
				this.selected = (localStorage.getItem('columns_event')).split(',')
			} else {
