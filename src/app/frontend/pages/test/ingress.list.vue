					...
					<div class="card-body table-responsive p-0">
						<b-table id="list" hover :items="items" :fields="fields" :filter="keyword" :filter-included-fields="filterOn" @filtered="onFiltered" :current-page="currentPage" :per-page="$config.itemsPerPage" :busy="isBusy" class="text-sm">
							...
							<template v-slot:cell(name)="data">
								<nuxt-link :to="{ path:'/view', query:{ context: currentContext(), group: 'Networking', crd: 'Ingress', name: data.item.name, url: `apis/networking.k8s.io/v1/namespaces/${data.item.namespace}/ingresses/${data.item.name}`, preurl: $router.currentRoute.fullPath}}">{{ data.value }}</nuxt-link>
							</template>
							...
							<template v-slot:cell(endpoints)="data">
								<ul class="list-unstyled mb-0">
									<li v-for="(d, idx) in data.item.endpoints" v-bind:key="idx" class="mr-1 font-weight-light text-md">{{ d }}</li>
								</ul>
							</template>
						</b-table>
					</div>
