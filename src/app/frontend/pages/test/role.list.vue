...
<div class="content-wrapper">
	...
	<div class="content-header">
		<div class="container-fluid">
			...
			<div class="row mb-2">
				<div class="col-sm-2"><h1 class="m-0 text-dark">Roles</h1></div>
				<!-- 검색 (namespace) -->
				<div class="col-sm-2">
				...
				<div class="col-sm-6 text-right dropdown">
					<b-button variant="primary" size="sm" @click="$router.push(`/create?context=${currentContext()}&group=Adiministrator&crd=Role`)">Create</b-button>
				</div><!--//END -->
			</div>
		</div>
	</div>
					...
					<div class="card-body table-responsive p-0">
						<b-table id="list" hover :items="items" :fields="fields" :filter="keyword" :filter-included-fields="filterOn" @filtered="onFiltered" :current-page="currentPage" :per-page="$config.itemsPerPage" :busy="isBusy" class="text-sm">
							...
							<template v-slot:cell(name)="data">
								<nuxt-link :to="{ path:'/view', query:{ context: currentContext(), group: 'Adiministrator', crd: 'Role', name: data.item.name, url: `apis/rbac.authorization.k8s.io/v1/namespaces/${data.item.namespace}/roles/${data.item.name}`, preurl: $router.currentRoute.fullPath}}">{{ data.value }}</nuxt-link>
							</template>
...
export default {
	...
	methods: {
		...
		query_All() {
			this.isBusy = true;
			axios.get(`${this.backendUrl()}/raw/clusters/${this.currentContext()}/apis/rbac.authorization.k8s.io/v1/namespaces/${this.$data.selectedNamespace}/roles`)
				.then((resp) => {
					this.items = [];
