  ...
  <div class="content-wrapper">
		<section class="content-header">
			<div class="container-fluid">
				...
				<div class="row mb-2">
					<div class="col-sm"><h1 class="m-0 text-dark"><span class="badge badge-info mr-2">R</span>Replica Sets <nuxt-link :to="{path:'/create', query: {group:'Workload', crd:'ReplicaSet'}}"><b-icon-plus-circle variant="secondary" font-scale="0.7"></b-icon-plus-circle></nuxt-link></h1></div>
				</div>
			</div>
		</section>
...
export default {
	...
	methods: {
		onRowSelected(items) {
			...
			if (this.isShowSidebar) this.viewModel = this.getViewLink('apps', 'replicasets', items[0].namespace, items[0].name)
		},
		// 조회
		query_All(d) {
			this.isBusy = true;
			this.$axios.get(this.getApiUrl("apps","replicasets",(d && d.namespace) ? d.namespace: this.selectNamespace(), "", d && d.labelSelector? `labelSelector=${d.labelSelector}`: ""))
				.then((resp) => {
					this.items = [];
