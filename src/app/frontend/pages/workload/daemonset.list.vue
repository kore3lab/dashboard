...
export default {
	...
	watch: {
		...
		selected() {
			...
			localStorage.setItem('columns_daemonset',this.selected)
		}
	},
	layout: "default",
	created() {
		...
		this.$nuxt.$on("navbar-context-selected", (ctx) => {
			if(localStorage.getItem('columns_daemonset')) {
				this.selected = (localStorage.getItem('columns_daemonset')).split(',')
			} else {
	...
	methods: {
		...
		onRowSelected(items) {
			if(items) {
				if(items.length) {
					...
					this.viewModel = this.getViewLink('apps', 'daemonsets', items[0].namespace, items[0].name)
		...
		query_All() {
			this.isBusy = true;
			this.$axios.get(this.getApiUrl("apps","daemonsets",this.selectedNamespace))
					.then((resp) => {
						this.items = [];
