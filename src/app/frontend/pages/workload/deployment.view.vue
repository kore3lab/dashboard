			...
			<div class="card card-secondary card-outline">
				<div class="card-body p-2">
					<dl class="row mb-0">
						...
						<span v-if="totalCpu !== 0" class="mb-1">Usage : {{ totalCpu }}</span>
						<span v-if="cpuRequests !== 0" class="mb-1">Requests : {{ cpuRequests.toFixed(2) }}</span>
						<span v-if="cpuLimits !== 0" class="mb-1">Limits : {{ cpuLimits.toFixed(2) }}</span>
						<span v-if="totalCpu === 0 && cpuRequests === 0 && cpuLimits === 0">-</span></dd>
						<dt class="col-sm-2">Memory</dt><dd class="col-sm-10">
						<span v-if="totalMemory !== 0" class="mb-1">Usage : {{ totalMemory | formatNumber }} Mi</span>
						<span v-if="memoryRequests !== 0" class="mb-1">Requests : {{ memoryRequests.toFixed(1) | formatNumber }} Mi</span>
						<span v-if="memoryLimits !== 0" class="mb-1">Limits : {{ memoryLimits.toFixed(1) | formatNumber }} Mi</span>
