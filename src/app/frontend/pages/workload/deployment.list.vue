					...
					<div class="card-body table-responsive p-0">
						<b-table id="list" hover :items="items" :fields="fields" :filter="keyword" :filter-included-fields="filterOn" @filtered="onFiltered" :current-page="currentPage" :per-page="$config.itemsPerPage" :busy="isBusy" class="text-sm">
							...
							<template v-slot:cell(name)="data">
								<nuxt-link :to="{ path:'/view', query:{ context: currentContext(), group: 'Workload', crd: 'Deployment', name: data.item.name, url: `apis/apps/v1/namespaces/${data.item.namespace}/deployments/${data.item.name}`, preurl: $router.currentRoute.fullPath}}">{{ data.value }}</nuxt-link>
							</template>
							<template v-slot:cell(images)="data">
								<ul class="list-unstyled mb-0">
									<li v-for="(image, idx) in data.item.images" v-bind:key="idx">{{ image }}</li>
								</ul>
							</template>
						</b-table>
					</div>
