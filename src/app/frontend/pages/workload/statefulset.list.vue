...
export default {
	...
	methods: {
		onRowSelected(items) {
			...
			if (this.isShowSidebar) this.viewModel = this.getViewLink('apps', 'statefulsets', items[0].namespace, items[0].name)
		},
		// 조회
		query_All(d) {
			this.isBusy = true;
			this.$axios.get(this.getApiUrl("apps","statefulsets",(d && d.namespace) ? d.namespace: this.selectNamespace(), "", d && d.labelSelector? `labelSelector=${d.labelSelector}`: ""))
				.then((resp) => {
					this.items = [];
