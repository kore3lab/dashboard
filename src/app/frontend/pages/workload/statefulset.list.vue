					...
					<div class="card-body table-responsive p-0">
						<b-table id="list" hover :items="items" :fields="fields" :filter="keyword" :filter-included-fields="filterOn" @filtered="onFiltered" :current-page="currentPage" :per-page="$config.itemsPerPage" :busy="isBusy" class="text-sm">
							...
							<template v-slot:cell(name)="data">
								<nuxt-link :to="{ path:'/view', query:{ context: currentContext(), group: 'Workload', crd: 'Setateful Set', name: data.item.name, url: `apis/apps/v1/namespaces/${data.item.namespace}/statefulsets/${data.item.name}`, preurl: $router.currentRoute.fullPath}}">{{ data.value }}</nuxt-link>
							</template>
...
export default {
	...
	methods: {
		...
		query_All() {
			this.isBusy = true;
			axios.get(`${this.backendUrl()}/raw/clusters/${this.currentContext()}/apis/apps/v1/namespaces/${this.$data.selectedNamespace}/statefulsets`)
				.then((resp) => {
					this.items = [];
