...
<div class="content-wrapper">
	...
	<div class="content-header">
		<div class="container-fluid">
			...
			<div class="row mb-2">
				<div class="col-sm-2"><h1 class="m-0 text-dark">Jobs</h1></div>
				<div class="col-sm-2">
				...
				<div class="col-sm-6 text-right dropdown">
					<b-dropdown text="Create" variant="primary" size="sm">
						<b-dropdown-item class="dropdown-item"><nuxt-link :to="{ path:'/create', query:{ context: currentContext(), group: 'Workload', crd: 'Job' } }">from Yaml</nuxt-link></b-dropdown-item>
					</b-dropdown>
				</div>
			</div>
		</div>
					...
					<div class="card-body table-responsive p-0">
						<b-table id="list" hover :items="items" :fields="fields" class="text-sm">
							<template v-slot:cell(name)="data">
								<nuxt-link :to="{ path:'/view', query:{ context: currentContext(), group: 'Workload', crd: 'Job', name: data.item.name, url: `job/namespace/${data.item.namespace}/name/${data.item.name}`}}">{{ data.value }}</nuxt-link>
							</template>
...
export default {
	...
	methods: {
		...
		query_All() {
			...
			axios.get(`${this.dashboardUrl()}/api/v1/job/${this.$data.selectedNamespace}?sortBy=d,creationTimestamp&context=${this.currentContext()}`)
				.then((resp) => {
					let data = []
					resp.data.jobs.forEach(el => {
						data.push({
