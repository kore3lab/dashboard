...
<div class="content-wrapper">
	...
	<div class="content-header">
		<div class="container-fluid">
			...
			<div class="row mb-2">
				<div class="col-sm-3"><h1 class="m-0 text-dark">Replication Controllers</h1></div>
				<!-- 검색 (namespace) -->
				<div class="col-sm-2">
				...
				<div class="col-sm-5 text-right">
					<b-dropdown text="Create" variant="primary" size="sm">
						<b-dropdown-item class="dropdown-item"><nuxt-link :to="{ path:'/create', query:{ context: currentContext(), group: 'Workload', crd: 'Replication Controller' } }">from Yaml</nuxt-link></b-dropdown-item>
					</b-dropdown>
				</div><!--// 버튼 -->
			</div>
		</div>
					...
					<div class="card-body table-responsive p-0">
						<b-table id="list" hover :items="items" :fields="fields" :filter="keyword" :filter-included-fields="filterOn" @filtered="onFiltered" :current-page="currentPage" :per-page="$config.itemsPerPage" :busy="isBusy" class="text-sm">
							...
							<template v-slot:cell(name)="data">
								<nuxt-link :to="{ path:'/view', query:{ context: currentContext(), group: 'Workload', crd: 'Replication Controller', name: data.item.name, url: `replicationcontroller/namespace/${data.item.namespace}/name/${data.item.name}`}}">{{ data.value }}</nuxt-link>
							</template>
...
export default {
	...
	methods: {
		...
		query_All() {
			this.isBusy = true;
			axios.get(`${this.dashboardUrl()}/api/v1/replicationcontroller/${this.$data.selectedNamespace}?sortBy=d,creationTimestamp&context=${this.currentContext()}`)
				.then((resp) => {
					this.items = [];
					resp.data.replicationControllers.forEach(el => {
						this.items.push({
