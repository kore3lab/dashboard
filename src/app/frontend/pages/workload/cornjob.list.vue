...
export default {
	...
	watch: {
		...
		selected() {
			...
			localStorage.setItem('columns_cronjob',this.selected)
		}
	},
	layout: "default",
	created() {
		...
		this.$nuxt.$on("navbar-context-selected", (ctx) => {
			if(localStorage.getItem('columns_cronjob')) {
				this.selected = (localStorage.getItem('columns_cronjob')).split(',')
			} else {
		...
		onRowSelected(items) {
			if(items) {
				if(items.length) {
					...
					this.viewModel = this.getViewLink('batch', 'cronjobs', items[0].namespace, items[0].name)
