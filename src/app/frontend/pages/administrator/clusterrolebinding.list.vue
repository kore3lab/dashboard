	...
	<div class="content-wrapper">
		<div class="content-header">
			<div class="container-fluid">
				...
				<div class="row mb-2">
					<!-- title & search -->
					<div class="col-sm"><h1 class="m-0 text-dark"><span class="badge badge-info mr-2">C</span>Cluster Role Bindings</h1></div>
					...
					<div class="col-sm-1 text-right">
						<b-button variant="primary" size="sm" @click="$router.push(`/create?context=${currentContext()}&group=Administrator&crd=ClusterRoleBinding`)">Create</b-button>
					</div>
				</div>
			</div>
		</div>
...
export default {
	...
	watch: {
		...
		selected() {
			...
			localStorage.setItem('columns_clusterrolebinding',this.selected)
		}
	},
	layout: "default",
	created() {
		...
		this.$nuxt.$on("navbar-context-selected", (ctx) => {
			if(localStorage.getItem('columns_clusterrolebinding')) {
				this.selected = (localStorage.getItem('columns_clusterrolebinding')).split(',')
			} else {
	...
	methods: {
		...
		onRowSelected(items) {
			if(items) {
				if(items.length) {
					...
					this.viewModel = this.getViewLink('rbac.authorization.k8s.io', 'clusterrolebindings', items[0].namespace, items[0].name)
		...
		query_All() {
			this.isBusy = true;
			this.$axios.get(this.getApiUrl("rbac.authorization.k8s.io","clusterrolebindings"))
					.then((resp) => {
						this.items = [];
