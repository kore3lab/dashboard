  ...
  <div class="content-wrapper">
    ...
    <div class="content-header">
      <div class="container-fluid">
        ...
        <div class="row mb-2">
          <div class="col-sm-3"><h1 class="m-0 text-dark"><span class="badge badge-info mr-2">C</span>Cluster Role Bindings</h1></div>
          <!-- 검색 (검색어) -->
              ...
              <div class="card-body table-responsive p-0">
                <b-table id="list" hover :items="items" :fields="fields" :filter="keyword" :filter-included-fields="filterOn" @filtered="onFiltered" :current-page="currentPage" :per-page="$config.itemsPerPage" :busy="isBusy" class="text-sm">
                  ...
                  <template v-slot:cell(name)="data">
                    <a href="#" @click="sidebar={visible:true, name:data.item.name, src:`${getApiUrl('rbac.authorization.k8s.io','clusterrolebindings')}/${data.item.name}`}">{{ data.value }}</a>
                  </template>
    ...
    <b-sidebar v-model="sidebar.visible" width="50em" right shadow no-header>
      <c-view crd="Cluster Role Binding" group="Administrator" :name="sidebar.name" :url="sidebar.src" @delete="query_All()" @close="sidebar.visible=false"/>
    </b-sidebar>
  </div>
</template>
<script>
...
export default {
  ...
  methods: {
    ...
    query_All() {
      this.isBusy = true;
      axios.get(this.getApiUrl("rbac.authorization.k8s.io","clusterrolebindings"))
          .then((resp) => {
            this.items = [];
