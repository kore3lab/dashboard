  ...
  <div class="content-wrapper">
    ...
    <div class="content-header">
      <div class="container-fluid">
        ...
        <div class="row mb-2">
          <div class="col-sm-2"><h1 class="m-0 text-dark"><span class="badge badge-info mr-2">C</span>Cluster Roles</h1></div>
          <!-- 검색 (검색어) -->
          ...
          <div class="col-sm-8 text-right">
            <b-button variant="primary" size="sm" @click="$router.push(`/create?context=${currentContext()}&group=Administrator&crd=ClusterRole`)">Create</b-button>
          </div><!--//END -->
        </div>
      </div>
    </div>
              ...
              <div class="card-body table-responsive p-0">
                <b-table id="list" hover :items="items" :fields="fields" :filter="keyword" :filter-included-fields="filterOn" @filtered="onFiltered" :current-page="currentPage" :per-page="$config.itemsPerPage" :busy="isBusy" class="text-sm">
                  ...
                  <template v-slot:cell(name)="data">
                    <a href="#" @click="sidebar={visible:true, name:data.item.name, src:`${getApiUrl('rbac.authorization.k8s.io','clusterroles')}/${data.item.name}`}">{{ data.value }}</a>
                  </template>
    ...
    <b-sidebar v-model="sidebar.visible" width="50em" right shadow no-header>
      <c-view crd="Cluster Role" group="Administrator" :name="sidebar.name" :url="sidebar.src" @delete="query_All()" @close="sidebar.visible=false"/>
    </b-sidebar>
  </div>
</template>
<script>
...
export default {
  ...
  methods: {
    ...
    query_All() {
      this.isBusy = true;
      axios.get(this.getApiUrl("rbac.authorization.k8s.io","clusterroles"))
          .then((resp) => {
            this.items = [];
