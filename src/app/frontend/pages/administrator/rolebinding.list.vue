					...
					<div class="card-body table-responsive p-0">
						<b-table id="list" hover :items="items" :fields="fields" :filter="keyword" :filter-included-fields="filterOn" @filtered="onFiltered" :current-page="currentPage" :per-page="$config.itemsPerPage" :busy="isBusy" class="text-sm">
							...
							<template v-slot:cell(name)="data">
								<nuxt-link :to="{ path:'/view', query:{ context: currentContext(), group: 'Adiministrator', crd: 'Role Binding', name: data.item.name, url: `apis/rbac.authorization.k8s.io/v1/namespaces/${data.item.namespace}/rolebindings/${data.item.name}`}}">{{ data.value }}</nuxt-link>
							</template>
