			...
			<div class="row mb-2">
				...
				<div class="col-sm-6 text-right dropdown">
					<b-dropdown text="Create" variant="primary" size="sm">
						<b-dropdown-item class="dropdown-item"><nuxt-link :to="{ path:'/create', query:{ context: currentContext(), group: 'Adiministrator', crd: 'Role Binding' } }">from Yaml</nuxt-link></b-dropdown-item>
					</b-dropdown>
				</div><!--//END -->
			</div>
		</div>
					...
					<div class="card-body table-responsive p-0">
						<b-table id="list" hover :items="items" :fields="fields" :filter="keyword" :filter-included-fields="filterOn" @filtered="onFiltered" :current-page="currentPage" :per-page="$config.itemsPerPage" :busy="isBusy" class="text-sm">
							...
							<template v-slot:cell(name)="data">
								<nuxt-link :to="{ path:'/view', query:{ context: currentContext(), group: 'Adiministrator', crd: 'Role Binding', name: data.item.name, url: `rolebinding/namespace/${data.item.namespace}/name/${data.item.name}`}}">{{ data.value }}</nuxt-link>
							</template>
...
export default {
	...
	methods: {
		...
		query_All() {
			this.isBusy = true;
			axios.get(`${this.dashboardUrl()}/api/v1/rolebinding/${this.$data.selectedNamespace}?sortBy=d,creationTimestamp&context=${this.currentContext()}`)
				.then((resp) => {
					this.items = [];
