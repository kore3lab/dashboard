...
export default {
	...
	watch: {
		selected() {
			...
			localStorage.setItem('columns_rolebinding',this.selected)
		}
	},
	created() {
		this.columnOpt = Object.assign([],this.fields)
		this.$nuxt.$on("navbar-context-selected", (ctx) => {
			if(localStorage.getItem('columns_rolebinding')) {
				this.selected = (localStorage.getItem('columns_rolebinding')).split(',')
			} else {
	...
	methods: {
		...
		onRowSelected(items) {
			if(items) {
				if(items.length) {
					...
					this.viewModel = this.getViewLink('rbac.authorization.k8s.io', 'rolebindings', items[0].namespace, items[0].name)
		...
		query_All() {
			this.isBusy = true;
			this.$axios.get(this.getApiUrl("rbac.authorization.k8s.io", "rolebindings", this.selectedNamespace))
					.then((resp) => {
						this.items = [];
