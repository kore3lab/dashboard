...
<div class="content-wrapper">
	...
	<div class="content-header">
		<div class="container-fluid">
			...
			<div class="row mb-2">
				<div class="col-sm-2"><h1 class="m-0 text-dark">Service Accounts</h1></div>
				<!-- 검색 (namespace) -->
				<div class="col-sm-2">
				...
				<div class="col-sm-6 text-right dropdown">
					<b-button variant="primary" size="sm" @click="$router.push(`/create?context=${currentContext()}&group=Adiministrator&crd=ServiceAccount`)">Create</b-button>
				</div><!--//END -->
			</div>
		</div>
	</div>
					...
					<div class="card-body table-responsive p-0">
						<b-table id="list" hover :items="items" :fields="fields" :filter="keyword" :filter-included-fields="filterOn" @filtered="onFiltered" :current-page="currentPage" :per-page="$config.itemsPerPage" :busy="isBusy" class="text-sm">
							...
							<template v-slot:cell(name)="data">
								<nuxt-link :to="{ path:'/view', query:{ context: currentContext(), group: 'Adiministrator', crd: 'Service Account', name: data.item.name, url: `api/v1/namespaces/${data.item.namespace}/serviceaccounts/${data.item.name}`}}">{{ data.value }}</nuxt-link>
							</template>
