...
<div class="content-wrapper">
	...
	<div class="content-header">
		<div class="container-fluid">
			...
			<div class="row mb-2">
				<div class="col-sm-3"><h1 class="m-0 text-dark">Persistent Volume Claims</h1></div>
				<!-- 검색 (namespace) -->
				<div class="col-sm-2">
				...
				<!-- 버튼 -->
				<div class="col-sm-5 text-right dropdown">
					...
					<div class="card-body table-responsive p-0">
						<b-table id="list" hover :items="items" :fields="fields" :filter="keyword" :filter-included-fields="filterOn" @filtered="onFiltered" :current-page="currentPage" :per-page="$config.itemsPerPage" :busy="isBusy" class="text-sm">
							...
							<template v-slot:cell(name)="data">
								<nuxt-link :to="{ path:'/view', query:{ context: currentContext(), group: 'Storage', crd: 'Persistent Volume Claim', name: data.item.name, url: `api/v1/namespaces/${data.item.namespace}/persistentvolumeclaims/${data.item.name}`, preurl: $router.currentRoute.fullPath}}">{{ data.value }}</nuxt-link>
							</template>
							...
							<template v-slot:cell(volume)="data">
								<nuxt-link :to="{ path:'/view', query:{ context: currentContext(), group: 'Storage', crd: 'Persistent Volume', name: data.item.volume, url: `persistentvolume/name/${data.item.volume}`}}">{{ data.value }}</nuxt-link>
							</template>
